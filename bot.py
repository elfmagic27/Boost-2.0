...
@bot.tree.command(name="redeempanel", description="Redeem your boost key or invite the bot to your server.")
async def redeempanel(ctx: discord.Interaction):
    embed = discord.Embed(
        title="⚡ Redeem Your Boost Key",
        description=(
            "Welcome! Use the buttons below to get started.\n\n"
            "**🔑 Redeem Key** — Enter your boost key and server ID to apply your boosts.\n"
            "**➕ Invite Bot** — Add the bot to your server before redeeming.\n\n"
            "> ⚠️ **Important:** The bot must be in your server before you redeem your key. "
            "Use **Invite Bot** first if you haven't already."
        ),
        color=0x7c3aed
    )
    embed.set_footer(text="Only you can see this message.")
    await ctx.response.send_message(embed=embed, view=RedeemPanelView(), ephemeral=True)
...
async def admin_save_secrets(request: Request, authorization: str = _Header(None)):
    ...
    ALLOWED = {
        ...
        "CAPSOLVER_API_KEY", "VOIDSOLVER_API_KEY", "ANTICAPTCHA_API_KEY",
        "KOVASOLVER_API_KEY", "SELLPASS_API_KEY",
    }
