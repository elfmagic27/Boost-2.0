...
def _apply_secrets_to_config():
    ...
    if _secret("ADMIN_PASSWORD"):
        config["admin_password"] = _secret("ADMIN_PASSWORD")
...
class Booster:
    ...
    def humanizer(self, token, custom_bio=None, custom_nick=None):
        ...
        ap = []
        # Only use the explicitly-passed value; never fall back to config defaults.
        # Empty / None → skip that API call and leave the account's existing value untouched.
        _bio  = (custom_bio  or '').strip()
        _nick = (custom_nick or '').strip()
        headers = {
            "authority": "discord.com",
            "scheme": "https",
            'method': 'PATCH',
...
async def extractor_page():
    with open("templates/extract.html", "r", encoding="utf-8") as f:
        return f.read()

# ── Token checker helpers ───────────────────────────────────────────────────
DISCORD_EPOCH     = 1420070400000  # Discord snowflake epoch in milliseconds
...
NITRO_TYPE_NAMES = {0: None, 1: "Nitro Classic", 2: "Nitro", 3: "Nitro Basic"}

def _snowflake_to_dt(snowflake_id: str) -> datetime.datetime:
...
# Checker sessions (separate from admin sessions): token -> expiry timestamp
_checker_sessions: dict = {}

@app.post("/api/check-auth")
async def api_check_auth(data: dict):
    """Validate checker password and return a short-lived session token."""
    password   = data.get("password", "")
    checker_pw = config.get("checker_password") or config.get("panel_password") or ""
    if not checker_pw or password != checker_pw:
        return JSONResponse({"ok": False}, status_code=401)
    sess = _sec.token_hex(32)
    _checker_sessions[sess] = time.time() + 7200  # 2-hour session
    return JSONResponse({"ok": True, "session": sess})

@app.post("/api/check-tokens")
async def api_check_tokens(request: Request):
    """Bulk-check Discord tokens and return rich account information."""
    from concurrent.futures import ThreadPoolExecutor

    # Accept session token (preferred) or raw password header (legacy fallback)
    session_tok = request.headers.get("X-Checker-Session", "")
    auth_pw     = request.headers.get("X-Checker-Auth", "")
    checker_pw  = config.get("checker_password") or config.get("panel_password") or ""

    authed = False
    if session_tok:
        exp = _checker_sessions.get(session_tok)
        if exp and time.time() <= exp:
            authed = True
        else:
            _checker_sessions.pop(session_tok, None)
    if not authed and auth_pw and checker_pw and auth_pw == checker_pw:
        authed = True  # legacy raw-password fallback
    ...
    # Discord error code → human-readable reason
    DISCORD_CODES = {
        0:     "Invalid token",
    ...
    def _classify_401(body_text: str) -> str:
        try:
            ...
            if "disabled" in msg.lower(): return "Account disabled"
            if "locked"   in msg.lower(): return "Account locked"
            if "verify"   in msg.lower(): return "Needs verification"
        except Exception:
            pass
        return "Invalid token"

    # ── Rate-limit-safe request helper ──────────────────────────────────────
    def _dget(url: str, hdrs: dict, timeout: int = 12, max_retries: int = 3):
        """GET with automatic 429 back-off (up to max_retries attempts)."""
        for attempt in range(max_retries):
            ...
            try:
                wait = float(r.json().get("retry_after", 0)) or (2 ** attempt * 2)
            except Exception:
                wait = 2 ** attempt * 2
            time.sleep(min(wait, 30))
        return r  # return last response if every attempt hit 429

    INTER_CALL_DELAY = 0.15   # seconds between the 3 Discord API calls inside check_one
    BATCH_SIZE       = 5       # tokens per parallel batch
    ...
    def check_one(line: str) -> dict:
        ...
        if len(parts) >= 3:
            token           = parts[2].strip()
            email_from_line = parts[0].strip() or None
        else:
            token           = line.strip()
            email_from_line = None
        ...
        result = {
            ...
            "nitro":                False,
            "nitro_type":           None,
            "nitro_started_at":     None,
            "nitro_expires_at":     None,
            "nitro_expires":        None,   # backward-compat alias
            "nitro_remaining_days": None,
            # Boosts
            "boost_slots":          0,      # backward-compat (free slots)
            "boost_slots_total":    0,
            "boost_slots_free":     0,
            "boost_slots_used":     0,
            "boosted_servers":      [],
        ...
        # ── Step 1: /users/@me ───────────────────────────────────────────
        try:
            me = _dget("https://discord.com/api/v9/users/@me", hdrs, timeout=12)
            if me.status_code == 401:
                reason = _classify_401(me.text)
                result["error"]      = reason
                result["error_type"] = (
                    "locked" if any(k in reason.lower() for k in ("lock", "disabl", "verif", "quarant"))
                    else "invalid"
                )
                return result
            ...
            result["global_name"]  = me_data.get("global_name") or uname
            result["user_id"]      = me_data.get("id")
            result["verified"]     = me_data.get("verified", False)
            result["phone"]        = bool(me_data.get("phone"))
            raw_flags              = me_data.get("flags", 0) or 0
            pub_flags              = me_data.get("public_flags", 0) or 0
            result["flags"]        = raw_flags | pub_flags
            result["premium_type"] = me_data.get("premium_type", 0) or 0
            result["email"]        = result["email"] or me_data.get("email")
            result["flags_list"]   = [
                name for bit, name in DISCORD_FLAGS_MAP.items() if result["flags"] & bit
            ]

            # Derive account creation date from snowflake ID
            if result["user_id"]:
                try:
                    created = _snowflake_to_dt(result["user_id"])
                    result["created_at"]       = created.strftime("%Y-%m-%dT%H:%M:%SZ")
                    result["account_age_days"] = (datetime.datetime.utcnow() - created).days
                except Exception:
                    pass

            # Quarantine flag (bit 20) → treat as locked
            if result["flags"] & (1 << 20):
        ...
        try:
            ...
            if slots_r.status_code == 200:
                ...
                free_slots = [s for s in slots_data if not s.get("premium_guild_subscription")]
                used_slots = [s for s in slots_data if s.get("premium_guild_subscription")]
                result["boost_slots_total"] = len(slots_data)
                result["boost_slots_free"]  = len(free_slots)
                result["boost_slots_used"]  = len(used_slots)
                result["boost_slots"]       = len(free_slots)  # backward compat
                result["nitro"]             = len(slots_data) > 0 or result["premium_type"] > 0
        ...
        # ── Step 3: Billing subscriptions (Nitro details + trial) ────────
        time.sleep(INTER_CALL_DELAY)
        try:
            sub_r = _dget(
            ...
            if sub_r.status_code == 200:
                ...
                if subs and isinstance(subs, list):
                    ...
                    result["nitro_started_at"] = period_start
                    result["nitro_expires_at"] = period_end
                    result["nitro_expires"]    = period_end  # backward compat
                    result["nitro"]            = True
                    result["nitro_type"]       = NITRO_TYPE_NAMES.get(result["premium_type"])

                    if period_end:
                        try:
                            exp = datetime.datetime.fromisoformat(period_end.replace("Z", "+00:00"))
                            now = datetime.datetime.now(datetime.timezone.utc)
                            result["nitro_remaining_days"] = max(0, (exp - now).days)
                        except Exception:
                            pass

                    # Trial detection via trial_ends_at field
                    trial_ends = sub.get("trial_ends_at")
                    if trial_ends:
                        result["trial_active"]     = True
                        result["trial_ends_at"]    = trial_ends
                        result["trial_started_at"] = period_start
                        items   = sub.get("items", [])
                        plan_id = items[0].get("plan_id", "") if items else ""
                        if "month" in plan_id.lower():
                            result["trial_type"] = "1 Month"
                        elif "week" in plan_id.lower():
                            result["trial_type"] = "2 Weeks"
                        else:
                            result["trial_type"] = "Trial"
                        try:
                            exp_t = datetime.datetime.fromisoformat(trial_ends.replace("Z", "+00:00"))
                            now   = datetime.datetime.now(datetime.timezone.utc)
                            result["trial_remaining_days"] = max(0, (exp_t - now).days)
                        except Exception:
                            pass
        except Exception:
            pass
    ...
    def run_batched() -> list:
        """Process tokens in small parallel batches with a pause between batches."""
        all_results = []
        for i in range(0, len(lines), BATCH_SIZE):
            batch = lines[i : i + BATCH_SIZE]
            with ThreadPoolExecutor(max_workers=len(batch)) as ex:
                all_results.extend(ex.map(check_one, batch))
            if i + BATCH_SIZE < len(lines):
                time.sleep(BATCH_PAUSE)
        return all_results

    loop    = asyncio.get_event_loop()
    results = await loop.run_in_executor(None, run_batched)
    return JSONResponse(results)

@app.get("/download-edited-files", include_in_schema=False)
async def download_edited_files():
    import os
    from fastapi.responses import FileResponse
    _zip = "/tmp/edited_files.zip"
    if not os.path.exists(_zip):
        import subprocess
        subprocess.run(["zip", _zip,
            "bot.py", "templates/check.html",
            "templates/admin.html", "replit.md"], check=True)
    return FileResponse(_zip, media_type="application/zip",
                        filename="edited_files.zip")
...
async def admin_login(request: Request):
    ...
    # Use ADMIN_PASSWORD if set; fall back to panel_password for backward compatibility
    admin_pw = config.get("admin_password") or config.get("panel_password") or ""
    if not admin_pw:
        return JSONResponse({"error": "No admin password configured"}, status_code=401)
    if pw != admin_pw:
        return JSONResponse({"error": "Unauthorized"}, status_code=401)
...
async def admin_get_secrets(authorization: str = _Header(None)):
    ...
    fields = [
        ...
        ("ADMIN_PASSWORD",     "Admin Panel Password",   _secret("ADMIN_PASSWORD")),
...
async def admin_save_secrets(request: Request, authorization: str = _Header(None)):
    ...
    ALLOWED = {
        ...
        "CHECKER_PASSWORD", "ADMIN_PASSWORD", "CAPSOLVER_API_KEY", "VOIDSOLVER_API_KEY",
